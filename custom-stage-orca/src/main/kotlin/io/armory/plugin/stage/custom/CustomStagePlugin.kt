...
class CustomStagePlugin(wrapper: PluginWrapper) : Plugin(wrapper) {
  private val logger = LoggerFactory.getLogger(CustomStagePlugin::class.java)

  override fun start() {
    logger.info("CustomStagePlugin.start()")
  }

  override fun stop() {
    logger.info("CustomStagePlugin.stop()")
  }
}
 ...
 */
@Extension
class CustomStageStage : StageDefinitionBuilder {
  ...
  override fun taskGraph(stage: StageExecution, builder: TaskNode.Builder) {
    builder.withTask("customStage", CustomStageTask::class.java)
  }
}

@Extension
class CustomStageTask(private val config: CustomStageConfig) : Task {

  private val log = LoggerFactory.getLogger(CustomStageTask::class.java)

  /**
   * This method is called when the task is executed.
   */
  override fun execute(stage: StageExecution): TaskResult {
    val context = stage.mapTo(CustomStageContext::class.java)
    val rand = Random()

...
/**
 * This test demonstrates that the CustomStagePlugin can be loaded by Orca
 * and that CustomStageStage's StageDefinitionBuilder can be retrieved at runtime.
 */
class CustomStageStageIntegrationTest : JUnit5Minutests {
  ...
  fun tests() = rootContext<OrcaPluginsFixture> {
    context("a running Orca instance") {
      ...
      test("CustomStageStage extension is resolved to the correct type") {
        val stageDefinitionBuilder = stageResolver.getStageDefinitionBuilder(
            CustomStageStage::class.java.simpleName, "customStage")

        expect {
          that(stageDefinitionBuilder.type).isEqualTo("customStage")
        }
      }

      test("CustomStageStage can be executed as a stage within a live pipeline execution") {
        ...
        expect {
          that(execution)
            .get { stages.first() }
            .and {
              get { type }.isEqualTo("customStage")
  ...
  data class Stage(val status: String, val context: CustomStageContext, val type: String)
}
